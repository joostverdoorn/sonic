...
describe("first", function() {
  it("should resolve with the first item in the list", function(done) {
    expect(Sonic.StateIterator.first(this.state)).toResolveWith(1, done);
  });

  it("should reject when the list is empty", function(done) {
    expect(Sonic.StateIterator.first(this.emptyState)).toReject(done);
  });

  it("should reject with the thrown error", function(done) {
    expect(Sonic.StateIterator.first(this.errorState)).toRejectWith(getError, done);
  });
});

describe("last", function() {
  it("should resolve with the last item in the list", function(done) {
    expect(Sonic.StateIterator.last(this.state)).toResolveWith(3, done);
  });

  it("should reject when the list is empty", function(done) {
    expect(Sonic.StateIterator.last(this.emptyState)).toReject(done);
  });

  it("should reject the error that is thrown by #get", function(done) {
    expect(Sonic.StateIterator.last(this.errorState)).toRejectWith(getError, done);
  });
});
...
describe("some", function() {
  it("should resolve with true if the predicate is true for some element", function(done) {
    expect(Sonic.StateIterator.some(this.state, function(x) { return x > 2; })).toResolveWith(true, done);
  });

  it("should resolve with false if the predicate if false for each element", function(done) {
    expect(Sonic.StateIterator.some(this.state, function(x) { return x > 3; })).toResolveWith(false, done);
  });

  it("should hit every element in the list until the predicate returns true", function(done) {
    var res = [];
    Sonic.StateIterator.some(this.state, function(x) {
      res.push(x);
      return x > 1;
    }).then(function() {
...
describe("every", function() {
  it("should resolve with true if the predicate is true for each element", function(done) {
    expect(Sonic.StateIterator.every(this.state, function(x) { return x < 4; })).toResolveWith(true, done);
  });

  it("should resolve with false if the predicate if false for some element", function(done) {
    expect(Sonic.StateIterator.every(this.state, function(x) { return x < 3; })).toResolveWith(false, done);
  });

  it("should hit every element in the list until the predicate returns false", function(done) {
    var res = [];
    Sonic.StateIterator.every(this.state, function(x) {
      res.push(x);
      return x < 2;
    }).then(function() {
      expect(res).toEqual([1,2]);
